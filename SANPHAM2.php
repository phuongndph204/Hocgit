...
<?php
    // Lấy dữ liệu từ biểu mẫu lọc
    $selectedCategories = isset($_POST['loaihang']) ? $_POST['loaihang'] : [];

    // Xây dựng điều kiện WHERE cho truy vấn SQL
    $whereClause = '';
    if (!empty($selectedCategories)) {
        $categoryIds = implode(',', $selectedCategories);
        $whereClause = "WHERE Ma_loai IN ($categoryIds)";
    }

    // Truy vấn để lấy sản phẩm theo loại hàng được chọn
    $query = "SELECT * FROM hanghoa $whereClause";
    $result = mysqli_query($connection, $query);
            ...
            echo '
                <div class="col-md-3 col-sm-6 col-xs-6 col-6 product-block">
                    ...
                    <div class="product-detail clearfix">
                        <div class="pro-text">
                            <a href="CHITIETSP.php?id=' . $id . '" style="color: black; font-size: 14px; text-decoration: none;" title="' . $row['ten_hang'] . '">
                                ' . $row['ten_hang'] . '
                            </a>
                        </div>
                        <div class="pro-price">
